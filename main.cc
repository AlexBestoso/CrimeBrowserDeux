...
void *graphicsCore(void *ptr){
	// Calculate aspect ratio to fix glut bug.
        ...
        ge.init("Crime Browser", SCR_WIDTH, SCR_HEIGHT);
...
void *networkCore(void *ptr){

	return ptr;
}

void *soundCore(void *ptr){

	return ptr;
}

void *storageCore(void *ptr){
...
int main(void){
	...
	e = pthread_create(&graphicsThread, NULL, graphicsCore, NULL);
	e = pthread_create(&networkThread, NULL, networkCore, NULL);
	e = pthread_create(&soundThread, NULL, soundCore, NULL);
	e = pthread_create(&storageThread, NULL, storageCore, NULL);
